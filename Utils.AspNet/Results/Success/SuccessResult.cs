...
/// <param name="success">A instância do sucesso para determinar o status code.</param>
/// <param name="value">O valor de sucesso a ser retornado na resposta.</param>
/// <param name="contentType">O content type HTTP da resposta</param>
    ...
    : IResult
{
    ...
    {
        var mappingService =
            httpContext.RequestServices.GetService<SuccessMappingService>();
        var mapping = mappingService?.GetMapping(success);
        var statusCode = (int)(mapping?.StatusCode ?? HttpStatusCode.OK);
