...
/// </summary>
internal sealed class DefaultErrorListProvider(IServiceProvider serviceProvider)
    : IErrorListProvider
{
    ...
    private ReadOnlyDictionary<Type, ErrorMapping> GetHttpMappings()
    {
        ...
        return mappingService?.Mappings
            ?? new ReadOnlyDictionary<Type, ErrorMapping>(new Dictionary<Type, ErrorMapping>());
    }
}
