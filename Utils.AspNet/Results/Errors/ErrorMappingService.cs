...
public class ErrorMappingService
{
    ...
    /// A chave é o <see cref="Type"/> do erro e o valor é o <see cref="ErrorMapping"/> correspondente.
    /// </summary>
    public ReadOnlyDictionary<Type, ErrorMapping> Mappings => _mappings.AsReadOnly();
    ...
    public ErrorMappingService(ILogger<ErrorMappingService> logger, IOptions<ResultOptions> options)
    {
        ...
        if (count > 0)
        {
            ...
            _logger.LogInformation("Adicionando {Count} mapeamento{s} de erro personalizado{s}.", count, plural, plural);
            foreach (var mapping in options.Value.ErrorMappings)
            {
                _mappings[mapping.ErrorType] = new ErrorMapping(mapping.StatusCode, mapping.Title, mapping.Type);
                _logger.LogDebug(
        ...
        _logger.LogInformation("Mapeamento de erros HTTP concluído. {Count} erros registrados.", _mappings.Count);
    }
